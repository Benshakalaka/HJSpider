...
class Spider(object):
    ...
    def getListenArticles(self, url):
        ...
        try:
            pageCount = Utils.getPageCount(soup)
        except Exception:
            self.logger.error('获取最大页数失败，默认1页')
            pageCount = 1
        ...
        for pageIndex in range(1, int(pageCount) + 1):
            ...
            for article in articlesList:
                full_url = Utils.listenHost + article.find(class_='big search_titles')["href"]
    ...
    def getListenArticleInfo(self, soup, url, oldurl):
        ...
        article.downloadUrl = Utils.listenHost + '/download/' + url.split('/')[-2][2:8] + '/'
    ...
    def run(self):
        ...
        # 先登录获取session
        self.session = Utils.userLogin(self.user_name, self.user_pass)
